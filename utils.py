...
def save_imgs(img, msk, msk_pred, key_points, gt_pre, id, save_path, datasets, threshold=0.5, test_data_name=None):
    ...
    # Set base filename
    base_filename = str(id)
    if test_data_name is not None:
        base_filename = test_data_name + '_' + base_filename
    
    # Check if file already exists
    if os.path.exists(save_path + base_filename + '_input.png'):
        return  # If file exists, return directly
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(save_path + base_filename + '_input.png')
    plt.close()
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(save_path + base_filename + '_gt_mask.png')
    plt.close()
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(save_path + base_filename + '_pred_mask.png')
    plt.close()
    
    # Save key points (maximum 3)
    for i in range(min(3, len(key_points))):
        kp = key_points[i]
        kp = kp.squeeze(0).permute(1,2,0).detach().cpu().numpy()
        plt.figure(figsize=(8, 8))
        plt.imshow(kp, cmap='jet')  # jetReflection vector
        plt.title(f'Key Point {i+1}')
        plt.axis('off')
        plt.colorbar()  # ，
        plt.tight_layout()
        plt.savefig(save_path + base_filename + f'_keypoint_{i+1}.png')
        plt.close()
    
    # Save multi-scale predictions (maximum 5)
    for i in range(min(5, len(gt_pre))):
        gt = gt_pre[i]
        gt = gt.squeeze(0).permute(1,2,0).detach().cpu().numpy()
        plt.figure(figsize=(8, 8))
        plt.imshow(gt, cmap='gray')
        plt.title(f'Scale Prediction {i+1}')
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(save_path + base_filename + f'_scale_{i+1}.png')
        plt.close()
    
# ，
    plt.figure(figsize=(8, 8))
    plt.imshow(img)
    plt.imshow(msk_pred, cmap='jet', alpha=0.5)
    plt.title('Prediction Overlay')
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(save_path + base_filename + '_overlay.png')
    plt.close()
# def save_imgs(img, msk, msk_pred, key_points, gt_pre, id, save_path, datasets, threshold=0.5, test_data_name=None):
    ...
    def __init__(self, data_name, train=True):
        if data_name == 'isic18':
            ...
            if train:
                ...
                self.mean = 85.2134
                self.std = 20.1582
        elif data_name == 'ham10000':
            # Using HAM10000 statistical mean and variance (0-1 range), multiply by 255 to convert to 0-255 range
            self.mean = np.array([0.7630, 0.5456, 0.5705]) * 255
            self.std = np.array([0.1409, 0.1526, 0.1697]) * 255
        elif data_name == 'brats2019':
# BraTS2019，[0,255]
            if train:
                self.mean = 0.0
                self.std = 1.0
