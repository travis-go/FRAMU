...
class setting_config:
    ...
    network = 'fractalmultiscale'  # Model architecture: FractalMultiScale
    model_config = {
        'num_classes': 1, 
        'input_channels': 3, 
        'base_channels': 7,  # Base channel number
        'fractal_depth': 3,  # Fractal recursion depth
        'patch_size': 2,     # Patch size for tokenization
        'expand_ratio': 1.5, # Channel expansion ratio
        'd_state': 10,       # State dimension
        'reflection_threshold': 0.5,  # Confidence threshold for reflection
        'apply_sigmoid': True,  # Apply sigmoid in wrapper
        # Knowledge base related configuration
        'use_knowledge_base': True
    }

    datasets = 'isic18'  # Dataset name
    if datasets == 'isic18':
        data_path = "./data/isic2018/"  # ISIC 2018 dataset path
    elif datasets == 'isic17':
        data_path = './data/isic2017/'  # ISIC 2017 dataset path
    elif datasets == 'ham10000':
        data_path = './data/ham10000/'  # HAM10000 dataset path
    elif datasets == 'BUSI':
        data_path = './data/BUSI/'  # BUSI dataset path
    else:
        raise Exception('Dataset not supported!')

    criterion = GT_BceDiceLoss(wb=1, wd=1)  # Loss function with original weights
    ...
    # Abductive Learning (ABL) reflection mechanism parameters
    abl = {
        'reflection_loss_weight': 0.1,  # Reflection loss weight
        'consistency_loss_weight': 0.05,  # Consistency loss weight
        'reflection_threshold': 0.5,  # Confidence threshold
        'size_loss_ratio': 0.75,
        'auxiliary_segmentation_loss': 0.05,  # Auxiliary segmentation loss weight
        'progressive_reflection': True,
        'use_fusion': True,  # Whether to use confidence fusion mechanism
        'apply_kb_correction': True  # Whether to apply knowledge base correction after fusion
    }
